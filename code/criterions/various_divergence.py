...
class VariousDivergence(CrossEntropyLoss):
    def __init__(self, args, padding_id=-100) -> None:
        ...
        elif self.kd_objective == "tvd":
            self.dist_func = self.compute_tvd
        elif self.kd_objective == "ToDi":
            self.dist_func = self.compute_ToDi
        else:
    ...
    def compute_tvd(
        ...
        teacher_logits = teacher_logits / self.kd_temp
        if use_tea_temp:
            teacher_logits = teacher_logits / self.tea_temp
        ...
        inf_mask = logits.isinf() | teacher_logits.isinf()
        tvd = tvd.masked_fill_(inf_mask, 0.0).sum(dim=-1)

        if reduction == "sum":
            pad_mask = target.eq(self.padding_id)
            tvd = tvd.masked_fill_(pad_mask, 0.0)
            tvd = tvd.sum()
            if log is not None:
                log["tvd"] = tvd
    ...
    def compute_ToDi(
        ...
        q = torch.softmax(logits, -1, dtype=torch.float32)         
        p = torch.softmax(teacher_logits, -1, dtype=torch.float32)  
    
        student_lprobs = torch.log_softmax(logits, -1, dtype=torch.float32)
        teacher_lprobs = torch.log_softmax(teacher_logits, -1, dtype=torch.float32)
    
        weight_p = torch.sigmoid(teacher_lprobs - student_lprobs).detach() 
        weight_q = torch.sigmoid(student_lprobs - teacher_lprobs).detach() 
    
        term1 = weight_p * p * (teacher_lprobs - student_lprobs)
        term2 = weight_q * q * (student_lprobs - teacher_lprobs)
        divergence = term1 + term2
    
        inf_mask = logits.isinf() | teacher_logits.isinf()
        divergence = divergence.masked_fill_(inf_mask, 0.0)
    
        divergence = divergence.sum(-1)
    
        if reduction == "sum":
            pad_mask = target.eq(self.padding_id)
            divergence = divergence.masked_fill_(pad_mask, 0.0)
